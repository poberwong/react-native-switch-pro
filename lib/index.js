...
export default class extends Component {
  ...
  animateSwitch = (toValue, callback = () => null) => {
    const { switchAnimation, animateTo } = this.state

    // make sure animations dont get triggered twice
    if (animateTo === toValue) {
      return
    }

    this.setState({animateTo: toValue})

    Animated.timing(switchAnimation,
      {
        toValue: toValue ? this.offset : -this.offset,
        duration: 200,
        easing: Easing.linear
      }
    ).start(() => {
      this.setState({animateTo: null})
      callback && callback()
    })
  }
